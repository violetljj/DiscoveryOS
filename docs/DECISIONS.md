...
## D-063：P2 因子必须替换同能力 control，并预冻结 paired estimands

- **日期**：2026-08-18
- **决定**：D-062 中“factor off 等于不加载对应 plugin”的实现被本决定 supersede。完整 trajectory 预检证明 `neither` 与 `Ada-only` 在 controller 合法选择 `STRUCTURAL_ESCAPE` 时缺少 provider，会系统性 fail closed。四臂现在始终各有一个 `LOCAL_REFINEMENT` 和 `STRUCTURAL_ESCAPE` provider：Ada factor 在 trajectory-unconditioned local control 与 Ada trajectory slice 间替换；EvoX factor在 strategy-unconditioned structural control 与 EvoX typed strategy slice 间替换。P2 protocol 使用六个既有 MVP-0 L2 consumed tasks、每 task/arm 两个 paired replicates、每 cell 最多 7 generation/evaluator calls，并在 task-resolution steps 上预声明 Ada main、EvoX main 和 `Y11-Y10-Y01+Y00` interaction。
- **原因**：移除 capability 会把“机制关闭”与“action 不可执行、generation opportunity 缺失”混为一谈，bootstrap-only fairness 无法发现这一点。相同能力 control 让四臂都能执行同一 action class、预算 reservation 和 evaluator path，同时只改变 trajectory conditioning 与 online strategy state。paired block 内先计算 contrast 再聚合，避免 arm-level median subtraction 和 `Ada+EvoX` 对 `neither` 的事后故事化解释。
- **后果**：旧 `3a93733` Profile identities 与 fairness digest 仅保留为暴露 blocker 的历史 mechanics snapshot，不得用于 P2 model calls。新 protocol 将正向方向、每个 estimand 至少一 task-resolution step、三项 one-sided exact paired sign tests 的 Holm `0.05`、无 free repair/resample、无预算转移、全 12 blocks 可评估以及 fail-closed terminal semantics 写入 create-once manifest。只有 clean-commit seal、provider/version/digest 绑定与零模型 task preflight 全部通过后才授权该 revision 的模型调用；结论上限仍是 consumed-task development signal。

## D-064：接受 P2 development protocol 的 create-once seal
...
## D-068：先建立 P2 infrastructure executability，再重开科学调用

- **日期**：2026-08-18
- **决定**：V3 后的顺序固定为 Infrastructure Autopsy → consumed L0-L2 Executability Gate → 新 create-once P2。`bounded_knapsack_beta-seed-17082601 / neither` 的 wall outlier 已通过 command receipts、ledger timing 和 Windows Kernel-Power events 归因为 `INFRA_FAILURE_HOST_SUSPEND_TIMING_CONTAMINATION`：异常两次 provider calls 在全体正常 duration 范围内，实际 Git/build/test/evaluator commands 为亚秒级，约 3881 秒落在跨 Modern Standby 的 runner 未细分区间。未来 factorial task 在首次 scientific generation 前必须证明 baseline evaluator 可评估、materialization 可重放、host suspend 未污染 CPU/wall baseline、provider timing/terminal provenance 完整且无未接纳 transport error；新协议必须预定义 timeout/retry/suspend/transport 的 `INFRA_FAILURE` 语义。
- **原因**：V3 已证明 profile fairness 和 create-once replay 不足以保证 cohort 可评估。继续增加 token、提高 wall ceiling或仅替换失败 task，会把基础设施污染与算法资源混合，并在已见 outcome 后形成 cohort tuning。异常 arm 的强证据不支持 provider/network 或算法计算归因；`load_balance_alpha` 的 baseline failure 仍需独立 autopsy，不能从本结论外推。
- **后果**：在 Executability Gate 通过前，P2 scientific generation budget 为零，P3 保持关闭。V3 root、9 个完整 blocks 和两个失败 task 的历史终态均不修改；不得续跑、补 block、删 task、挑 cohort或重算 estimand。新 P2 必须使用独立、统一、事前冻结的 task admission 规则与新 seal；本决定只提高基础设施诊断与 admission 要求，不建立 Ada/EvoX value、generalization、superiority 或 production claim。完整证据见 [`P2_INFRASTRUCTURE_AUTOPSY.md`](P2_INFRASTRUCTURE_AUTOPSY.md)。

## D-069：将 `load_balance_alpha` baseline failures 归类为 host low-power infrastructure contamination

- **日期**：2026-08-18
- **决定**：接受 `load_balance_alpha` 两个 V3 baseline failures 的 0-generation-call autopsy。seed `17082602` 的最低层分支为 `TIMEOUT:test`，seed `17082601` 为 `TIMEOUT:repository_setup`；两者随后都被 wall reconciliation 包装为 `BUDGET_EXHAUSTED:wall_seconds:prior=...`，再被 block runner 泛化为 `BASELINE_EVALUATOR_NOT_EVALUABLE:neither`。独立 Kernel-Power/Power-Troubleshooter events 分别把其约 3.6 小时与 3.5 小时 wall interval 绑定到连续 Modern Standby 和 hibernate。正式共同 failure class 为 `ENVIRONMENT_INFRA_FAILURE_HOST_LOW_POWER_STATE_CONTAMINATION`，两个 subtypes 分别为 test-time suspend 与 repository-setup hibernate deadline expiry。
- **原因**：两个 alpha materializations 的 file/tree identities 均匹配 manifest；full evaluator exact replay 分别从原冻结 commit 在临时副本运行，均返回 finite `score=0.3565120065120065`、`valid=1.0`。Seal manifest 也已有同值的两次 baseline replay。alpha 与可评估 beta 的 initial source、public tests、environment lock、commands、runner/parser、reference 和 intermediate assets 相同，只有 evaluator 的冻结 `CASES` 不同；beta 的早期 blocks 在低功耗窗口前可评估。因此证据反对 `TASK_NOT_BASELINE_EVALUABLE`、`EVALUATOR_CONTRACT_DEFECT` 与 `MATERIALIZATION_DEFECT`。
- **后果**：不修 V3、不 prune 原运行留下的 worktree provenance、不补 baseline、不删 alpha，也不把 exact replay 改写为 V3 scientific recovery。Executability Gate 仍属于下一阶段且尚未实现；其设计必须保留 materialized-byte identity、逐 command timing/terminal、host sleep/hibernate provenance 与原始 failure chain，不能只依赖被 reconciliation/block terminal 泛化后的 label。P2 generation budget 继续为零，P3 保持关闭。完整证据见 [`P2_LOAD_BALANCE_BASELINE_AUTOPSY.md`](P2_LOAD_BALANCE_BASELINE_AUTOPSY.md)。

## D-070：以 preventer + detector 形式冻结 Executability Gate V1

- **日期**：2026-08-18
- **决定**：正式实现 `DISCOVERYOS_EXECUTABILITY_GATE_V1`，并接受其在 consumed L0-L2 上的零 generation-call qualification。Windows session 必须在任何 baseline/scientific execution 前取得 `SYSTEM_REQUIRED + EXECUTION_REQUIRED` power-inhibition lease，查询并拒绝已存在的 standby/hibernate state；untouched baseline 必须在 immutable tree clone 上两次经过完整 evaluator/parser，得到 finite、`VALID`、确定性 score；scientific callback 后必须在 lease 释放前完成 power-event、timing 与 provider receipt reconciliation。任一 provenance 不完整、低功耗 overlap、timeout、materialization drift、baseline invalidity、provider retry/timeout/transport error、未解释 wall 超限或 lease acquire/release failure均 fail closed，block 不得进入 factorial estimand。
- **原因**：V3 三个 invalid blocks 已由 D-068/D-069 归因到 host low-power contamination。Gate 的第一次 live development qualification 又在 lease 建立前约 31 秒观测到真实 Modern Standby entry，并正确拒绝运行，证明 lease 不能追溯性消除既有 low-power state，因此加入 lease 后、baseline 前的 active-state precheck。Host 醒来后的 canonical qualification 在六个 V3 L2 consumed tasks 上均完成 exact tree replay 与两次 baseline `VALID`；七个 L0 adversarial fixtures 对 awake、suspend、hibernate、timeout、unexplained wall、lease failure 与 materialization drift 全部命中预期 class。两次 baseline 的正常未解释 runner wall 为 `1.024–1.062s`，故 V1 冻结 absolute allowance `2.0s` 与 fraction `20%` 取大者；V3 的约 `3881.575s` gap 仍会明确失败。
- **后果**：Gate 当前 claim ceiling 仅为 `MECHANICS_AND_CONSUMED_DEVELOPMENT_ONLY`。本阶段 0 generation/provider calls、0 fresh/SEALED assets，未修改 V3 root、未读取或重算其 9/12 outcomes，未设计/seal 新 P2，也未决定 future recovery/backfill。下一阶段才允许事前设计新的 P2 revision；P3 仍须等待新的、正向且可重放的 factorial evidence。完整协议与证据见 [`P2_EXECUTABILITY_GATE.md`](P2_EXECUTABILITY_GATE.md) 及 [`evidence/P2_EXECUTABILITY_GATE_QUALIFICATION_V1.json`](evidence/P2_EXECUTABILITY_GATE_QUALIFICATION_V1.json)。

## D-071：冻结 P2 V4 的独立 family cohort、最大 N、恢复语义与继承统计

- **日期**：2026-08-18
- **决定**：P2 V4/P2-R1 保持 V3 的 Ada、EvoX、Ada×EvoX 科学问题，以及四臂 capability-matched Profiles、task-resolution estimands、正向一侧 exact sign tests、三项 Holm `0.05`、一 resolution-step median gate 和 P3 interaction/noninferiority logic；V3 outcomes 不参与 effect、task、threshold、factor 或 tie-rate 设计。纯理论 `Binomial(N,0.5)` 计算显示，在最严格 `0.05/3` 门下，`N=12/18/24` 分别需要 `11/14/18` 个正向 non-tie blocks；在真实正向概率 `0.75` 时条件功效分别约 `0.158/0.519/0.607`，故冻结最大 `N=24`。一个 primary block 必须是一个 distinct Benchmark Bank family 的单一 DEV instance；同 family 的两个 instance/seed 不得冒充独立 blocks。cohort 固定为 `6 R0 / 6 R1 / 12 R2` external contract-derived DEV families，并在任一 generation call 前完成 immutable materialization、两次 full-evaluator baseline replay、`24/24` Executability Gate 和 clean-worktree manifest seal。
- **原因**：继续使用 V3 六题会形成 outcome-informed replication；把当前 32 instances 当 32 independent units 又会把仅 16 个 external DEV families 伪装成足够样本。12 blocks 对三项 Holm 检验理论功效过低，而 24 是本次比较范围内首个在 `p=0.80` 达到约 0.81 条件功效的 maximum。整 cohort 先通过 Gate 可在调用前消除 baseline/materialization blocker；窄 recovery 则避免机器已证明的 host suspend 再次永久污染整轮，同时不为 provider、算法、candidate 或资源失败提供免费重试。
- **后果**：当前 Bank `3 R0 / 3 R1 / 10 R2 = 16` 个合格 external DEV families 不足，必须先零模型扩充至少 `+3/+3/+2` families。V4 runner、cohort manifest 和 create-once root 尚未实现，scientific generation authority 为零。未来只允许 Gate receipt 机器证明的 host low-power、power-lease failure 或零 generation-call transient host materialization/environment failure 触发 whole-block censor/restart；每 block 最多一次、全局最多两次，永不只补失败 arm。provider/algorithm/candidate/resource/protocol failure 不恢复。账本必须分开 scientific 与 infra-censored calls/tokens，同时保留 total paid cost。完整冻结规则见 [`P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md`](P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md)。

## D-072：以零模型 contract-derived adapter 补足 P2 V4 的 24-family reservoir

- **日期**：2026-08-18
- **决定**：在 pinned AlgoTune commit `dff9914c10800c7a031c9e8c3d4d1c8cd1b38906` 上新增版本化 adapter `discoveryos.algotune_p2v4_contract_dev.v1`。R0 提升全部三个缺失注册 family；R1 对五个 catalogued candidates 使用 `SHA256(P2_V4_PROTOCOL_ID || family_id)` 的冻结顺序并选择前三名 `least_squares / fft_convolution / min_weight_assignment`；R2 仅新增覆盖当前 Bank 缺口的 `vertex_cover / tsp`。每 family 提供两个确定性 DEV instances，但 V4 primary inference 仍只允许 SHA-ranked 单一 instance。Registry 扩为 49 families，external contract-derived DEV reservoir 恰为 `6 R0 / 6 R1 / 12 R2 = 24` families，digest 为 `742637df6a5de643e17a9db19070a11c5be311c3ba06d56e54035ec5737e13cf`。
- **原因**：D-071 要求 family-level 独立单位，不能用原 16 families 的 32 instances 冒充 24 blocks。现有 R0 候选恰好补足缺口；R1 hash rank 消除人工挑题；Vertex Cover 与 TSP 分别补充 cover selection 与 sequence/cycle optimization，而不引入新的运行时依赖。独立标准库 evaluator 在 bounded DEV cases 上检查 input immutability、结构、feasibility/exact objective 或数值 residual；它们明确不复用或冒充上游 NumPy/SciPy/OR-Tools evaluator。
- **后果**：新增 16/16 instances 的 public/evaluator 执行通过，digest tamper 与 invalid candidate fail closed；总计 0 model/provider calls、0 SHADOW/SEALED assets、0 fresh consumption。此结果只建立 V4 candidate reservoir，不证明 residual headroom、24/24 cohort executability、Ada/EvoX value 或 scientific admission。下一步才允许实现 deterministic selector、full-cohort Gate、recovery/cost ledger、aggregate/replay 与 adversarial tests；在单一 instance 选择、headroom 和 24/24 Gate 完成且 create-once manifest 封存前，generation authority 继续为零。完整边界见 [`BENCHMARK_BANK_V1.md`](BENCHMARK_BANK_V1.md) 与 [`P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md`](P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md)。

## D-073：P2 V4.1 在 cohort seal 前以确定性 opcode cost 取代 wall-clock DEV score

- **日期**：2026-08-18
- **决定**：D-072 的 24-family identity 与 D-071 的科学设计保持不变，但三个 local AlgoTune DEV evaluator regimes 被 `discoveryos.algotune_p2v41_deterministic_dev.v1 / DISCOVERYOS_P2V41_DETERMINISTIC_OPCODE_DEV_V1` supersede。新 score 是 materialized `algorithm.py` 在固定 CPython 3.11 下执行的负 opcode count，方向仍为 maximize；task-specific resolution 只由 untouched baseline 和两个固定-case lookup calibration programs 事前确定。两 calibration programs 必须正确、输入不变、严格改善且 score 不同，并提供至少四个 resolution steps，否则 materialization fail closed。它们只证明 DEV headroom，不进入 scientific candidate set。
- **原因**：首个零模型 probe 对同一 untouched `affine_transform_2d_dev_alpha` 得到两个有效但不同的 wall-clock scores（`0.7650524060312658` 与 `0.7697636824782145`，差 `0.004711276446948687`），已知无法满足 Executability Gate V1 的 `1e-12` baseline determinism。继续写或运行 24/24 runner 只会制造预知的 protocol failure。Opcode count 保留“更少确定性执行成本更好”的开发语义，同时可精确 replay；固定-case lookup 只作为可达 calibration/reference material，不被包装成一般算法 reference。
- **后果**：Registry digest 更新为 `8428268400f6c23c13e58b0476b08c25e0870980feb081c7481063efd7b97a0a`；48/48 DEV instances 均通过两次 finite/VALID/identical score replay、positive resolution 和四步以上 headroom checks。模型/provider calls、SHADOW/SEALED openings 和 fresh consumption均为零。该修订不改变 Ada/EvoX estimands、Holm、`N=24`、infra recovery 或 P3 gate，也不建立 24/24 cohort executability或 scientific admission。完整边界并入 [`P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md`](P2_FACTORIAL_V4_PREMODEL_STATISTICAL_SEAL.md)。
