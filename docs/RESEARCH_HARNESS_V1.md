...
The V1 matched-resource P2 seal failed its first independent-worktree execution-authority check before any model call because the Harness bundle digest was checkout-line-ending sensitive; V1 also lacked a bound execution/settlement/replay entrypoint. V2 normalizes CRLF/LF for Harness content identity and binds the exact 12-block runner, create-once terminals, factorial reducer and replay. A real consumed-task four-arm block now passes zero-model construction and fairness, but V2 model calls remain unauthorized until the new implementation commit is sealed from a clean detached worktree. See [`P2_FACTORIAL_DEVELOPMENT_PROTOCOL.md`](P2_FACTORIAL_DEVELOPMENT_PROTOCOL.md).
