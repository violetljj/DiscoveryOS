...
3. 下一步只重新定义并冻结 `neither / Ada-only / EvoX-only / Ada+EvoX` 四臂 Profile，验证同一 Executor、Evaluator、Budget、Ledger 和资源预留表面的零模型公平性。通过后才能封存 P2 development protocol；不得在此阶段新增搜索机制或使用 fresh 题。
