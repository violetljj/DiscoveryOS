...
35. CMI Search Value R1 V1 在 commit `1a3e3b5`、manifest `0a82137cdda8d406885b276e20b04308515e78ea5bf461a60c2a5e20e32114e7` 上于首个 terminal task receipt 前触发 resource-envelope blocker：四次实际 provider calls 合计 `85,348` tokens，超过冻结 `80,000` arm ceiling；最后一次 generation 正确记录 `BUDGET_EXHAUSTED`，但 aggregation 未结算为 terminal `NOT_EVALUABLE`。Failure receipt SHA-256 为 `fde8384ea996e1b588f5ba62b04b7fd71d7da85675eedac2d4b96cb0b7a9f438`；V1 无 scientific output，partial first task 与完整 cohort 均不得重用。V2 仅做 validity repair：新 salt、新的无筛选六任务 cohort、按四调用 horizon 将 ceiling 冻结为 `120,000`，并让任何 over-budget observation 从 matched metrics 排除且 arm 标为 non-evaluable；CMI、applicability、schedule、evaluator 与三道科学 gate 全部不变。V2 已实现但尚未封存或执行。详见 [`CMI_SEARCH_VALUE_R1.md`](CMI_SEARCH_VALUE_R1.md)。
