...
- 长时间测试、benchmark 或研究运行禁止“启动后无证据等待”。启动前必须先做不消耗科学资产的 preflight/smoke，至少验证命令、环境、输入、输出根、权限、依赖、资源上限和最小执行路径；SEALED/fresh 资产不得用于这类调试。
- 进程启动后必须先做早期健康检查，确认主/子进程存活、日志或心跳更新、输出文件/收据数量或时间戳增长，以及 CPU、内存、GPU/显存和磁盘使用符合预期；健康证据成立后才可进入较长的有界等待。
- 运行期间必须持续留下每个 task/seed/arm/iteration 的 start、progress、terminal、耗时、资源、异常和失败原因；开发评估的科学结果可按协议实时查看，blind/SEALED 运行也必须可观察运行健康，但不得为了监控而提前暴露受保护的 scientific outcome。
- 若现有 runner 不能提供可观察的进度、心跳、分轮 terminal 或持久化错误，应先补充最小监控仪表或外层 wrapper，再执行长任务；不得把“控制台很安静”当作可接受的长时间运行方式。
- 对无心跳、无输出增长、进程退出、连续无效、资源越界、磁盘危险或明确错误设置预定义的异常条件并 fail fast。停止前保留日志、已有收据和部分 artifact；不得因 wrapper 超时、控制台暂无输出或单次轮询未变就判定卡死，应先核查进程活动、日志、时间戳与输出增长。
- 监控节奏按任务耗时和异常风险设置：启动后先进行一次短间隔健康检查，稳定后退避为长间隔事件等待或工具原生 wait，不做高频空轮询，也不允许一次长等待跨过整个早期故障发现窗口。
