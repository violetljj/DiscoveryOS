...
R1.0-A synthetic admission 的冻结协议、结果与限制见 [ASHA admission 记录](docs/ASHA_ADMISSION.md)。R1.0-B 已实现 bounded LLM Local Patch、一次 mechanical repair、generation provenance/token accounting 与 matched-token real-code benchmark；详见 [Local Patch admission 记录](docs/LLM_LOCAL_PATCH_ADMISSION.md)。[R1.0-BR-A fresh readmission](docs/LLM_LOCAL_PATCH_RELIABILITY.md) 已在 8 个冻结 task 上完成：reliability PASS，但 One-shot 与 Iterative 均为 8/8、paired 0 胜 8 平 0 负，search-value FAIL，因此正式 verdict 继续为 `LLM_LOCAL_PATCH_NOT_ADMITTED`。单 active branch 的 autonomous mechanics loop 现已由 ledger-backed state projector、deterministic controller、unified executor 和 anytime settlement 接通；这只把状态上调到 `DISCOVERYOS_AUTONOMOUS_SEARCH_LOOP_MECHANICS_READY`，仍是 `DISCOVERYOS_SEARCH_VALUE_NOT_YET_ESTABLISHED` / `DISCOVERYOS_PRODUCTION_NOT_READY`。下一步按 [Search-Value MVP-0 冻结协议](docs/SEARCH_VALUE_MVP0.md) 运行 Vanilla one-shot vs DiscoveryOS unified loop，不继续扩展 controller 或先建远端队列。[R1.0-SP-A](docs/SEARCH_POLICY_ADMISSION.md) 只保留为防止任务筛选污染的最小 benchmark guard，目前仍是 `SEARCH_POLICY_PROTOCOL_ONLY`。总体架构、内部机制与 external challenger 的边界见 [架构与路线图](docs/ARCHITECTURE.md)。
