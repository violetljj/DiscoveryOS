...
R1.0-A synthetic admission 的冻结协议、结果与限制见 [ASHA admission 记录](docs/ASHA_ADMISSION.md)。R1.0-B 已实现 bounded LLM Local Patch、一次 mechanical repair、generation provenance/token accounting 与 matched-token real-code benchmark；详见 [Local Patch admission 记录](docs/LLM_LOCAL_PATCH_ADMISSION.md)。[R1.0-BR-A fresh readmission](docs/LLM_LOCAL_PATCH_RELIABILITY.md) 已在 8 个冻结 task 上完成：reliability PASS，但 One-shot 与 Iterative 均为 8/8、paired 0 胜 8 平 0 负，search-value FAIL，因此正式 verdict 继续为 `LLM_LOCAL_PATCH_NOT_ADMITTED`。单 active branch 的 autonomous mechanics loop 现已由 ledger-backed state projector、deterministic controller、unified executor 和 anytime settlement 接通；MVP-0 结果为 `SEARCH_VALUE_MVP0_FAIL`，预算可达性已在后续 BR 中修复，但总体仍是 `DISCOVERYOS_SEARCH_VALUE_NOT_YET_ESTABLISHED` / `DISCOVERYOS_PRODUCTION_NOT_READY`。[Strategy Integration SI-1](docs/STRATEGY_INTEGRATION_SI1.md) 已把 Shinka-style parent selection 与 novelty rejection 映射成 DiscoveryOS 原生 primitive，并在 consumed tasks 上完成四臂消融：mechanics 为 `SHINKA_PARENT_NOVELTY_MECHANICS_READY`，duplicate evaluation 下降带来 `DEVELOPMENT_SIGNAL_POSITIVE`，但 parent diversity、最终收益和后续 marginal search value 均未增加，因此不构成 search-value 或机制 admission。[来源与适配映射](docs/SHINKA_MECHANISM_MAPPING.md) 明确禁止把 Shinka 作为黑盒或越过现有 evaluator/budget authority。[R1.0-SP-A](docs/SEARCH_POLICY_ADMISSION.md) 只保留为防止任务筛选污染的最小 benchmark guard，目前仍是 `SEARCH_POLICY_PROTOCOL_ONLY`。总体架构、内部机制与 external challenger 的边界见 [架构与路线图](docs/ARCHITECTURE.md)。
